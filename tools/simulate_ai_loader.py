...
if __name__ == "__main__":
    print("""
╔══════════════════════════════════════════════════════════════════════╗
║  v2.7.0 极致瘦核心入口验证 · 模拟 gpt-5.6-sol 启动器加载行为          ║
║  目标：证明主入口真的只加载 3.9KB，其他 KB 按需加载                  ║
╚══════════════════════════════════════════════════════════════════════╝
    """)
    ...
    print("="*70)
    print("""
  ✅ 主入口 AGENTS.md 真实大小: 3.9 KB（3866 字节）
  ✅ 典型任务 token 消耗: 入口 1-2K + 1-2 个 KB = 总 3-5K tokens
  ✅ KB-33 仅在边界外请求时才加载（拒绝恢复场景）
  ✅ 日常 CrackMe/漏洞/渗透任务：零 KB-33 加载

  v2.6.4 → v2.7.0 优化:
    - AGENTS.md: 8KB → 3.9KB (-51%)
    - Codex: 5.8KB → 3.6KB (-38%)
    - Claude: 5.3KB → 3.6KB (-32%)
    - KB-33: 14KB → 6.3KB (-55%)
    - 入口 token 总消耗: ~5.6K → ~3.1K (-45%)
